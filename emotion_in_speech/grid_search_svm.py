...
class SVM_parameter_search:
    ...
    def grid_search(self, classifier, str_classifier):
        parameters = {
        ...
        self.plot_grid(clf, self.save, 'none',
                       str_classifier, self.feature, str(self.kernel[0]))

    def grid_search_pca(self, classifier,  str_classifier):
        ...
        clf = GridSearchCV(pipe, param_grid=parameters, cv=3, scoring='accuracy', n_jobs=-1, verbose=1, return_train_score=False)
        clf.fit(self.x, self.y)
        self.plot_grid(clf, self.save, 'pca',
                       str_classifier, self.feature, self.kernel[0])

    def plot_grid(self, clf, save, method_str, classifier_str, feature,
                  kernel):
        ...
        plt.savefig(save + classifier_str + feature +
                    '_' + self.name + '_' + method_str + '_' + kernel)
        plt.close()
        with open(save + classifier_str +
                  feature + '_' + self.name + '_' +
                  method_str + '_' +
                  'best_parameters' +
                  '_' + kernel
                  + '.csv', 'w') as csvfile:
