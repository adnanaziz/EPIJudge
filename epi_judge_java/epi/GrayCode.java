...
public class GrayCode {
  ...
  public static void grayCodeWrapper(TestTimer timer, int numBits) throws TestFailureException {
    timer.start();
    ...
    for (int i = 1; i < result.size(); i++)
      if (!differsByOneBit(result.get(i - 1), result.get(i))) {
        if (result.get(i - 1).equals(result.get(i))) {
          ...
          throw new TestFailureException("Two adjacent entries differ by more than 1 bit");
        }
      }

    Set<Integer> uniq = new HashSet<>(result);
    if (uniq.size() != result.size()) {
      throw new TestFailureException("Not all entries are distinct: found "
