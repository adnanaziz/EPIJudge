...
public class SmallestSubarrayCoveringSet {
  ...
  public static int findSmallestSubarrayCoveringSetWrapper(
      TestTimer timer, List<String> v, Set<String> s) throws TestFailureException {
    ...
    if (result.start < 0 || result.start >= v.size() || result.end < 0 || result.end >= v.size()
        || result.start > result.end)
      throw new TestFailureException("Index out of range");

    for (int i = result.start; i <= result.end; i++) copy.remove(v.get(i));
